...
const agentMascotHide = document.getElementById('agent-mascot-hide');
const agentMascotMenu = document.getElementById('agent-mascot-menu');
const agentMascotMenuClose = document.getElementById('agent-mascot-menu-close');
...
const spModalMascot = document.getElementById('sp-modal-mascot');
const spModalMascotPreview = document.getElementById('sp-modal-mascot-preview');
const spModalMascotClear = document.getElementById('sp-modal-mascot-clear');
const spModalMascotVisible = document.getElementById('sp-modal-mascot-visible');
...
const catModalContexte = document.getElementById('cat-modal-contexte');
...
let _catEditFromManagePopup = false;
let _selectedCatColor = '#3b82f6';
...
catColorGrid.addEventListener('click', (e) => {
    ...
    _selectedCatColor = swatch.dataset.color;
});
...
// Chaque streamCtx capture conversationId + history + abortController au démarrage du stream.
...
let conversationLastActivity = null;
let conversationTitle = null;
let firstPrompt = null;
let conversationStarted = false;
let currentSystemPrompt = null;
let _editingAgentAvatar = null;
...
function readAvatarAsSquareDataUrl(file) {
    return new Promise((resolve, reject) => {
        ...
        image.onerror = () => {
            URL.revokeObjectURL(objectUrl);
            reject(new Error('Image avatar invalide'));
        };
        image.src = objectUrl;
    });
}

function clearMediaPreview(container, emptyText) {
    if (!container) return;
    container.innerHTML = '';
...
function renderAgentMediaPreview() {
    clearMediaPreview(spModalAvatarPreview, 'Aucun avatar');
    if (_editingAgentAvatar?.data) {
        const img = document.createElement('img');
        img.src = _editingAgentAvatar.data;
        img.alt = 'Aperçu de l’avatar';
        spModalAvatarPreview.replaceChildren(img);
    }

    clearMediaPreview(spModalMascotPreview, 'Aucune mascotte');
    if (_editingAgentMascot?.data) {
        if (_editingAgentMascot.type === 'lottie') {
            spModalMascotPreview.textContent = 'Lottie JSON prêt à être affiché';
        } else {
            const video = document.createElement('video');
            video.src = _editingAgentMascot.data;
            video.autoplay = true;
            video.loop = true;
            video.muted = true;
...
function destroyMascotWidget() {
    if (!agentMascotContent) return;
    if (agentMascotContent._lottieAnimation) {
        agentMascotContent._lottieAnimation.destroy();
        agentMascotContent._lottieAnimation = null;
    }
    agentMascotContent.replaceChildren();
}

function positionMascotWidget(position) {
    if (!agentMascotWidget) return;
    const mascotWidth = parseFloat(getComputedStyle(agentMascotWidget).width) || 132;
    const mascotHeight = parseFloat(getComputedStyle(agentMascotWidget).height) || 164;
    const maxLeft = Math.max(8, window.innerWidth - mascotWidth - 8);
    const maxTop = Math.max(8, window.innerHeight - mascotHeight - 8);
    const hasVisiblePosition = position
    ...
    if (hasVisiblePosition) {
        _agentMascotUsesDefaultPosition = false;
        agentMascotWidget.style.left = `${position.left}px`;
        agentMascotWidget.style.top = `${position.top}px`;
        agentMascotWidget.style.right = 'auto';
        agentMascotWidget.style.bottom = 'auto';
    } else {
        _agentMascotUsesDefaultPosition = true;
...
async function updateSelectedAgentVisuals(filename = chatAgentSelect?.value || '') {
    ...
    if (chatAgentAvatar) {
        ...
        chatAgentAvatar.removeAttribute('src');
    }
    if (!filename) {
    ...
    if (chatAgentAvatar && agent.avatar?.data) {
        ...
        chatAgentAvatar.hidden = false;
    }
    if (!agent.mascotte?.data || agent.mascotteVisible === false || _agentMascotSessionHidden) {
        if (agentMascotWidget) agentMascotWidget.hidden = true;
        return;
    }
...
function syncChatAgentSelect() {
    ...
    const selectedValue = spSelect?.value || '';
    chatAgentSelect.innerHTML = '<option value="">Aucun agent</option>';
    if (spSelect) {
        Array.from(spSelect.options).forEach((option) => {
            if (!option.value) return;
            const quickOption = document.createElement('option');
            quickOption.value = option.value;
            quickOption.textContent = option.textContent;
            quickOption.dataset.contenu = option.dataset.contenu || '';
            chatAgentSelect.appendChild(quickOption);
        });
    }
    chatAgentSelect.value = selectedValue;
    if (chatAgentStatus) {
        const activeOption = chatAgentSelect.selectedOptions[0];
        chatAgentStatus.textContent = activeOption?.value
            ? `Actif : ${activeOption.textContent}`
            : 'Aucun agent actif';
    }
    _agentMascotSessionHidden = false;
    updateSelectedAgentVisuals(selectedValue);
}
...
let pendingLoadingFiles = [];
let _pendingLoadId = 0;
...
const STREAM_ERROR_CONTENT = 'Une erreur est survenue. Réessayez de générer la réponse, changez de modèle si nécessaire.';

// Un stream est « actif » (= visible et synchrone avec les globals) si la conv
// affichée est celle du stream ET que l'history capturée n'a pas été remplacée
// par un loadConversation entre-temps.
function isStreamActive(streamConvId, streamHistory) {
    return streamConvId === conversationId && streamHistory === conversationHistory;
}
...
// Bloque les schémas dangereux (javascript:, data:, vbscript:) dans les href.
...
function applyTheme(mode) {
    if (mode === 'auto') {
        ...
        themeToggle.innerHTML = '&#9681; Thème auto';
    } else {
        document.body.classList.remove('theme-auto');
        document.body.classList.toggle('dark', mode === 'dark');
        themeToggle.innerHTML = mode === 'dark' ? '&#9790; Thème sombre' : '&#9788; Thème clair';
    }
}

const savedTheme = localStorage.getItem('minou-theme') || 'light';
...
convSearch.addEventListener('input', async () => {
    ...
    if (query && !_fullTextsLoaded) {
        _fullTextsLoaded = true;
...
function _trackLoad(name, reader) {
    const entry = { id: ++_pendingLoadId, name, reader };
    pendingLoadingFiles.push(entry);
    renderAttachPreview();
    updateSendButton();
    return entry;
}
...
function _untrackLoad(entry) {
    const idx = pendingLoadingFiles.indexOf(entry);
    if (idx >= 0) pendingLoadingFiles.splice(idx, 1);
}

function _isLoadCancelled(entry) {
    return pendingLoadingFiles.indexOf(entry) === -1;
}

function _cancelAllPendingLoads() {
    for (const entry of pendingLoadingFiles) {
...
function renderAttachPreview() {
    ...
    pendingLoadingFiles.forEach((entry) => {
        const chip = document.createElement('div');
        chip.className = 'attach-file-chip attach-file-chip-loading';

        const spinner = document.createElement('span');
        spinner.className = 'attach-file-chip-spinner';
        spinner.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="spin"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>';

        const name = document.createElement('span');
        name.className = 'attach-file-chip-name';
        name.textContent = entry.name;
        name.title = `${entry.name} — chargement…`;

        const removeBtn = document.createElement('button');
        removeBtn.className = 'attach-file-chip-remove';
        removeBtn.textContent = '×';
        removeBtn.title = 'Annuler';
        removeBtn.addEventListener('click', () => {
            try { entry.reader.abort(); } catch {}
            _untrackLoad(entry);
            renderAttachPreview();
...
document.getElementById('share-menu-md').addEventListener('click', () => {
    ...
    if (currentSystemPrompt) md += `**Agent** : ${currentSystemPrompt.nom}  \n`;
    md += `**Recherche web** : ${currentSearchModel ? 'Activée' : 'Désactivée'}  \n`;
...
<div class="meta-value">${totalInputTokens.toLocaleString('fr-FR')} entr\u00e9e \u00b7 ${totalOutputTokens.toLocaleString('fr-FR')} sortie</div>
...
// Le panneau de réglages reste fermé au démarrage et s'ouvre uniquement à la
// demande via l'engrenage.
...
function _resolveImageCost(tarif, usage, imageCount, format, imageParams) {
    if (usage?.cost_real != null && usage.cost_real >= 0) {
        ...
        // (totalImageCost garde sa signification de "coût de génération d'images")
...
if (chatAgentSelect) {
    chatAgentSelect.addEventListener('change', () => {
        spSelect.value = chatAgentSelect.value;
        spSelect.dispatchEvent(new Event('change'));
        if (chatAgentSelect.value) {
            const opt = spSelect.selectedOptions[0];
            currentSystemPrompt = {
                nom: opt?.textContent || 'Agent',
                contenu: spTextarea.value || opt?.dataset.contenu || ''
            };
        } else {
...
function hideAgentMascot() {
    _agentMascotSessionHidden = true;
    if (agentMascotWidget) agentMascotWidget.hidden = true;
    if (agentMascotMenu) agentMascotMenu.hidden = true;
}

agentMascotMenuClose?.addEventListener('click', hideAgentMascot);

if (agentMascotWidget) {
    let dragState = null;
    agentMascotWidget.addEventListener('contextmenu', (event) => {
        event.preventDefault();
        if (!agentMascotMenu) return;
        agentMascotMenu.hidden = false;
        agentMascotMenu.style.left = `${Math.min(event.clientX, window.innerWidth - 180)}px`;
        agentMascotMenu.style.top = `${Math.min(event.clientY, window.innerHeight - 48)}px`;
    });
    agentMascotWidget.addEventListener('pointerdown', (event) => {
        if (event.button !== 0 || agentMascotWidget.hidden) return;
        if (agentMascotWidget.hidden) return;
        _agentMascotUsesDefaultPosition = false;
        const rect = agentMascotWidget.getBoundingClientRect();
        dragState = { offsetX: event.clientX - rect.left, offsetY: event.clientY - rect.top };
        agentMascotWidget.setPointerCapture(event.pointerId);
        agentMascotWidget.classList.add('dragging');
    });
    agentMascotWidget.addEventListener('pointermove', (event) => {
        if (!dragState) return;
        const left = Math.min(Math.max(8, event.clientX - dragState.offsetX), window.innerWidth - agentMascotWidget.offsetWidth - 8);
        const top = Math.min(Math.max(8, event.clientY - dragState.offsetY), window.innerHeight - agentMascotWidget.offsetHeight - 8);
        agentMascotWidget.style.left = `${left}px`;
        agentMascotWidget.style.top = `${top}px`;
        agentMascotWidget.style.right = 'auto';
        agentMascotWidget.style.bottom = 'auto';
    });
    agentMascotWidget.addEventListener('pointerup', async (event) => {
        if (!dragState) return;
        dragState = null;
        agentMascotWidget.classList.remove('dragging');
        agentMascotWidget.releasePointerCapture?.(event.pointerId);
        const filename = chatAgentSelect?.value;
        if (filename) {
            const agent = await readSystemPrompt(filename);
            if (agent) {
                const rect = agentMascotWidget.getBoundingClientRect();
                await writeSystemPrompt(filename, { ...agent, mascottePosition: { left: rect.left, top: rect.top } });
            }
        }
    });
}
...
window.addEventListener('resize', () => {
    if (!agentMascotWidget?.hidden) {
        const rect = agentMascotWidget.getBoundingClientRect();
        const isOutsideViewport = rect.left < 8
            || rect.top < 8
            || rect.right > window.innerWidth - 8
            || rect.bottom > window.innerHeight - 8;
        if (!_agentMascotUsesDefaultPosition && !isOutsideViewport) return;
        positionMascotWidget(null);
    }
});

document.addEventListener('click', (event) => {
    if (agentMascotMenu && !agentMascotMenu.contains(event.target)) {
        agentMascotMenu.hidden = true;
    }
});
...
spTextarea.addEventListener('input', () => {
    if (currentSystemPrompt) {
        currentSystemPrompt.contenu = spTextarea.value;
    }
...
function updateWebSearchBtn() {
    ...
    if (activeModel) {
        ...
        if (model && WEB_SEARCH_EDITEURS.includes(model.editeur)) {
            ...
            webSearchBtn.classList.toggle('active', builtIn || webSearchEnabled);
...
if (webSearchBtn) {
    webSearchBtn.addEventListener('click', () => {
        ...
        if (hasBuiltInWebSearch(currentModel)) return;
...
function updateCanvasBtn() {
    ...
    const activeTextModel = currentModel;
...
/**
 * À appeler juste après un `conversationHistory.push({ role: 'assistant', ... })`
...
function refreshCatBar() {
    ...
    if (activeCategoryId) {
        const cat = readCategory(activeCategoryId);
        if (cat) {
            catSelectLabel.textContent = (cat.icone || '') + ' ' + cat.nom;
        } else {
            activeCategoryId = null;
            catSelectLabel.textContent = 'Tous les projets';
        }
    } else {
        catSelectLabel.textContent = 'Tous les projets';
    }
    ...
    allOpt.className = 'cat-select-option' + (!activeCategoryId ? ' active' : '');
    allOpt.dataset.value = '';
    allOpt.textContent = 'Tous les projets';
    catSelectDropdown.appendChild(allOpt);

    for (const cat of cats) {
        const opt = document.createElement('div');
        opt.className = 'cat-select-option' + (activeCategoryId === cat.id ? ' active' : '');
...
function updateEmptyChatCategory() {
    ...
    if (activeCategoryId) {
        ...
        if (cat) {
            ...
            el.textContent = `Cette conversation sera rangée dans le projet ${cat.nom}.`;
            return;
        }
    }
...
function updateActiveCatColor() {
    ...
    const catId = conversationId ? currentConversationCategory : activeCategoryId;
    if (catId) {
...
catSelectDropdown.addEventListener('click', (e) => {
    ...
    if (!opt) return;
    activeCategoryId = opt.dataset.value || null;
    catSelectDropdown.style.display = 'none';
    catSelect.classList.remove('open');
    refreshCatBar();
    refreshConvList();
    if (!conversationId) {
        currentConversationCategory = activeCategoryId;
        updateActiveCatColor();
        promptInput.focus();
    }
});
...
function openCatModal(catId, fromManage) {
    ...
    _catEditFromManagePopup = !!fromManage || (catModalOverlay.style.display !== 'none');
    hideEmojiPicker();
    if (catId) {
        const cat = readCategory(catId);
        if (!cat) return;
        catModalTitle.textContent = 'Modifier le projet';
        catModalNom.value = cat.nom;
        catModalContexte.value = cat.contexte || '';
        ...
        catModalTitle.textContent = 'Nouveau projet';
        catModalNom.value = '';
        catModalContexte.value = '';
    ...
    catModalBack.style.display = _catEditFromManagePopup ? '' : 'none';
    catModalCancel.style.display = _catEditFromManagePopup ? 'none' : '';
...
async function renderCatManageList() {
    ...
    if (cats.length === 0) {
        catManageList.innerHTML = '<div class="cat-manage-empty">Aucun projet pour le moment.<br>Créez-en un ci-dessous.</div>';
        return;
    }
    // Compter les conversations par projet
...
catModalSave.addEventListener('click', async () => {
    ...
    writeCategory(id, {
        nom,
        couleur: _selectedCatColor,
        icone: catModalIcone.value || '?',
        contexte: catModalContexte.value.trim()
    });
    hideEmojiPicker();
    if (_catEditFromManagePopup) {
        catManageEditView.style.display = 'none';
...
catModalDelete.addEventListener('click', async () => {
    ...
    if (!await customConfirm('Supprimer ce projet ? Les conversations seront retirées du projet.', { icon: 'delete', danger: true, okLabel: 'Supprimer' })) return;
...
const EDITEUR_LABELS = {
    ...
    ollamaCloud: 'Ollama Cloud', lmstudio: 'LM Studio', omniroute: 'OmniRoute'
};
...
const EDITEUR_ORDER = ['openai', 'anthropic', 'google', 'mistral', 'perplexity', 'deepseek', 'grok', 'zai', 'groq', 'ollamaCloud', 'omniroute', 'openrouter', 'ollama', 'lmstudio'];
const EDITEUR_ICONS = {
    openai: 'OpenAI.svg', anthropic: 'Anthropic.svg', google: 'Google.svg',
    mistral: 'Mistral.svg', perplexity: 'Perplexity.svg',
    deepseek: 'DeepSeek.svg', grok: 'Grok.svg', zai: 'Z.ai.svg', groq: 'Groq.svg',
    openrouter: 'OpenRouter.svg', ollama: 'Ollama.svg', ollamaCloud: 'Ollama.svg', omniroute: 'Other.svg', lmstudio: 'LMStudio.svg'
};
function _editeurLabel(editeur) {
    return EDITEUR_LABELS[editeur] || editeur;
}
function _editeurIcon(editeur) {
    return EDITEUR_ICONS[editeur];
}
function _editeurGroupHeaderHtml(editeur) {
    const icon = _editeurIcon(editeur);
    const iconHtml = icon ? `<img class="custom-select-group-icon" src="images/${icon}" alt="">` : '';
    return `<span class="custom-select-group-label">${iconHtml}<span>${escHtml(_editeurLabel(editeur))}</span></span>`;
}
...
function hasProviderKey(editeur) {
    ...
    if (editeur === 'ollamaCloud') {
        const overlay = document.getElementById('apikeys-modal-overlay');
        const key = document.getElementById('apikey-ollamaCloud');
        const url = document.getElementById('apikey-ollamaCloudUrl');
        const hasLive = !!(key && key.value.trim()) && !!(url && url.value.trim());
        if (overlay && overlay.style.display && overlay.style.display !== 'none') return hasLive;
        return !!(API_KEYS && API_KEYS.ollamaCloud && API_KEYS.ollamaCloudUrl);
    }
    if (editeur === 'omniroute') {
        const overlay = document.getElementById('apikeys-modal-overlay');
        const key = document.getElementById('apikey-omniroute');
        const url = document.getElementById('apikey-omnirouteUrl');
        const hasLive = !!(key && key.value.trim()) && !!(url && url.value.trim());
        if (overlay && overlay.style.display && overlay.style.display !== 'none') return hasLive;
        return !!(API_KEYS?.omniroute && API_KEYS?.omnirouteUrl);
    }
    const overlay = document.getElementById('apikeys-modal-overlay');
...
function updateTriggerDisplay(selectEl) {
    ...
    if (triggerIcon) {
        const icon = model && _editeurIcon(model.editeur);
        if (icon) {
...
initConfig().then(async () => {
    ...
    console.error('[Kiro] Initialisation interrompue :', error);
    if (typeof window.__kiroSplashReady === 'function') window.__kiroSplashReady();
});
...
function populateModelSelect() {
    ...
    if (prevModel) {
        ...
        if (isImg) {
            currentImageModel = prevModel; currentModel = null; currentSearchModel = null;
            _switchTab('image', false);
        } else if (isSrch) {
            currentSearchModel = prevModel; currentModel = null; currentImageModel = null;
            _switchTab('search', false);
        } else {
            currentModel = prevModel; currentImageModel = null; currentSearchModel = null;
...
function checkApiKeyForModel(modelId, lookupFn) {
    ...
    if (editeur === 'omniroute') {
        if (!hasProviderKey(editeur)) {
            showModelAlert('Clé API et URL du fournisseur personnalisé manquantes. Renseignez-les dans Configuration.');
            return false;
        }
        return true;
    }
...
function updateSendButton() {
    if (isStreaming) {
        ...
        const isLoadingFiles = pendingLoadingFiles.length > 0;
    ...
    attachBtn.classList.toggle('has-files', pendingImages.length > 0 || pendingFiles.length > 0);
    updateEnhanceBtn();
}
...
sendBtn.addEventListener('click', () => {
    if (isStreaming) {
        ...
        if (conversationId) _activeStreams.delete(conversationId);
        isStreaming = false;
...
function showEmptyPlaceholder() {
    ...
    ph.innerHTML = '<img src="images/Hinata.webp" alt="Hinata" class="empty-chat-logo"><p class="empty-chat-text">Sélectionnez une conversation ou démarrez-en une nouvelle en saisissant votre message ci-dessous.</p><div id="empty-chat-category" class="empty-chat-category" style="display:none"></div>';
...
function resetConversation() {
    ...
    currentImageModel = null;
    currentSearchModel = null;
    currentConversationCategory = activeCategoryId || null;
    pendingImages = [];
    pendingFiles = [];
    _cancelAllPendingLoads();
    originalPromptBeforeEnhance = null;
    isEnhancing = false;
    attachPreview.innerHTML = '';
...
// Erreurs Kiro déjà rédigées en français, à afficher telles quelles sans préfixe "Erreur :".
function _isFriendlyKiroError(msg) {
    if (!msg) return false;
...
function showErrorAlert(explanation, rawError) {
    if (!explanation) {
        if (_isFriendlyKiroError(rawError)) return customAlert(rawError, 'error');
...
function showMissingModelBanner(missingId) {
    ...
    console.info(`[Kiro] Modèle "${safe}" introuvable lors du chargement de la conversation, sélection réinitialisée.`);
}
...
function updateTokenDisplay() {
    ...
    tokenInfo.textContent = `↑ ${totalInputTokens.toLocaleString('fr-FR')} ↓ ${totalOutputTokens.toLocaleString('fr-FR')} Tokens`;

    const displayCost = totalCost + totalImageCost + totalAudioCost + totalTitleCost;
    if (displayCost > 0) {
...
function _mergeConvData(data, convId, { overrides = {}, deltas = null } = {}) {
    const merged = {
        ...
        workFolder: data.work_folder || data.workFolder || null,
...
async function _saveConvById(convId, history, deltas) {
    ...
    if (convId === conversationId && !_activeStreams.has(convId)) {
...
function saveConversation() {
    ...
    const data = {
        ...
        lastActivity: conversationLastActivity,
        totalInputTokens,
        totalOutputTokens,
        totalCost,
        totalImageCost,
        ...
        workFolder: window.WorkMode?.getConversationAccess?.() || null,
...
// Capture conversationId/history au démarrage : la requête API au modèle de titre est asynchrone,
...
async function maybeGenerateTitle(convIdArg, historyArg, modelHint) {
    const _titleConvId = convIdArg || conversationId;
    const _titleHistory = historyArg || conversationHistory;
    ...
    async function _applyTitle(newTitle, costDelta) {
        ...
        if (conversationId === _titleConvId) {
            conversationTitle = newTitle;
            if (costDelta) totalTitleCost += costDelta;
            updateChatHeader();
            updateTokenDisplay();
        }
        if (!refreshConvListItem(filename)) refreshConvList();
    ...
    const modelId = AUDIO_SETTINGS.titleModel || modelHint || currentModel || currentImageModel || currentSearchModel;
    if (!modelId) return;
...
function addMessage(role, content, citations, generationTime, thinking, outputTokens, model) {
    ...
    if (isImageResponse) {
        ...
        copyBtn.addEventListener('click', () => {
            images.forEach((img, idx) => {
                ...
                a.href = src;
                const ext = (img.mimeType || 'image/png').split('/')[1] || 'png';
                a.download = images.length > 1 ? `kiro-image-${idx + 1}.${ext}` : `kiro-image.${ext}`;
                a.click();
            });
            copyIcon.innerHTML = checkSvg;
    ...
    if (role === 'assistant' && !hasImages) {
        ...
        ttsBtn.addEventListener('click', (e) => {
            ...
            if (currentTtsAudio) {
                if (currentTtsAudio === 'system') {
                    window.speechSynthesis.cancel();
                } else if (typeof currentTtsAudio === 'function') {
...
async function regenerateLastResponse() {
    ...
    const lastAssistantForRewind = conversationHistory.length > 0
    ...
    // Re-générer
    conversationLastActivity = new Date().toISOString();
    isStreaming = true;
    currentAbortController = new AbortController();
    updateSendButton();

    const _streamConvId = conversationId;
    const _streamHistory = conversationHistory;
    _activeStreams.set(_streamConvId, { conversationId: _streamConvId, history: _streamHistory, abortController: currentAbortController });
    ...
    if (currentImageModel && !getImageModelEditeur(currentImageModel)) {
    ...
    if (currentImageModel) {
        ...
        const prompt = buildImagePrompt(getTextFromContent(lastUserMsg.content), conversationHistory);

        const regenRefImages = collectReferenceImages(conversationHistory);

        const _regenImgFormat = document.getElementById('image-format-select').value;
        const _regenImgParams = getImageParams();
        const _regenImgModel = currentImageModel;
        generateImage(
            currentImageModel,
            prompt,
            (result) => {
                if (!_activeStreams.has(_streamConvId)) return;
                _activeStreams.delete(_streamConvId);
                ...
                if ((!result.images || result.images.length === 0) && !result.text) {
                    if (_streamConvId === conversationId) {
                ...
                if (_isActive) {
                    ...
                    totalCost += _resolved.tokenCost;
                    totalImageCost += _resolved.imageCost;
        ...
        streamModel(
            regenTextModel,
            _limitHistoryImages(conversationHistory, getMaxHistoryImages()),
            (chunk) => {
            ...
            (usage, citations) => {
                ...
                // `_streamConvId === conversationId` couvre aussi le cas où l'utilisateur
                // est revenu sur la conv après avoir navigué (l'isStreaming global a
                ...
                if (_isActive) {
                    ...
                    if (usage) {
                        totalInputTokens += usage.input_tokens || 0; totalOutputTokens += usage.output_tokens || 0;
                        totalCost += segCost;
            ...
            _effectiveSp1,
            webSearchEnabled && !hasBuiltInWebSearch(regenTextModel),
            (thinkChunk) => {
                fullThinking += thinkChunk;
                if (_streamConvId !== conversationId) return;
                ensureThinkBlock();
...
function startEditMessage(wrapper, msgDiv) {
    // Trouver l'index dans conversationHistory
    const allWrappers = Array.from(chatContainer.querySelectorAll('.message-wrapper'));
    let histIdx = -1;
    let wIdx = 0;
    for (let i = 0; i < conversationHistory.length; i++) {
        if (conversationHistory[i].type === 'model-switch') continue;
    ...
    if (!textEl) return;
    const originalText = getTextFromContent(conversationHistory[histIdx].content);
    ...
    confirmBtn.addEventListener('click', async () => {
        ...
        for (let i = histIdx + 1; i < conversationHistory.length; i++) {
        ...
        conversationHistory[histIdx].content = newText;
        conversationHistory.splice(histIdx + 1);
        ...
        if (currentImageModel) {
            ...
            const _editImgModel = currentImageModel;
            generateImage(
                currentImageModel,
                newText,
                (result) => {
        ...
        streamModel(
            ...
            _effectiveSp2,
            webSearchEnabled && !hasBuiltInWebSearch(activeTextModel),
            (thinkChunk) => {
                fullThinking += thinkChunk;
                if (_streamConvId !== conversationId) return;
                ensureThinkBlock();
...
function handleApiError(assistantDiv, userText, fileNames, modelName, err) {
    ...
    const finalize = (explanation) => {
        ...
        if (last && last.role === 'assistant') {
            ...
            conversationHistory.push({ role: 'assistant', content: errorContent, error: true });
        }
        if (conversationId) saveConversation();
        addRegenBtn();
        const el = assistantDiv.querySelector('.message-text');
...
async function sendMessage() {
    ...
    if (pendingLoadingFiles.length > 0) {
        const names = pendingLoadingFiles.map(e => e.name).join(', ');
        customAlert(`Patientez quelques instants : ${names} ${pendingLoadingFiles.length > 1 ? 'sont encore' : 'est encore'} en cours de chargement.`, 'wait');
        return;
    }
    ...
    if (!conversationId) {
        ...
        if (!_firstLabel) {
            if (pendingImages.length > 0) {
                ...
                _firstLabel = pendingFiles.length === 1
                    ? `(${pendingFiles[0].name})`
                    : '(fichiers joints)';
            } else {
                _firstLabel = '(message)';
    ...
    if (spSelect.value) {
        const opt = spSelect.selectedOptions[0];
        currentSystemPrompt = { nom: opt.textContent, contenu: spTextarea.value || opt.dataset.contenu };
    } else if (spTextarea.value.trim()) {
        currentSystemPrompt = { nom: 'Personnalisé', contenu: spTextarea.value };
    } else {
        currentSystemPrompt = null;
    }
    ...
    const _streamConvId = conversationId;
    const _streamHistory = conversationHistory;
    const _streamCtx = { conversationId: _streamConvId, history: _streamHistory, abortController: currentAbortController };
    _activeStreams.set(_streamConvId, _streamCtx);
    ...
    if (currentImageModel) {
        ...
        generateImage(
            ...
            // onDone
            (result) => {
                const _ctx = _activeStreams.get(_streamConvId);
                if (!_ctx) return;
                _activeStreams.delete(_streamConvId);
                const _isActive = isStreamActive(_streamConvId, _streamHistory);
        ...
        const _workContext = window.WorkMode?.enabled ? await window.WorkMode.getPromptContext() : '';
        const _effectiveSp3 = effectiveSystemPrompt(spContent) + (window.WorkMode?.getInstructions?.() || '') + _workContext;
        streamModel(
            activeTextModel,
            _limitHistoryImages(conversationHistory, getMaxHistoryImages()),
            // onChunk
            (chunk) => {
                ...
                const _isVisible = (_streamConvId === conversationId) && _streamCtx.assistantDiv && _streamCtx.assistantDiv.isConnected;
            ...
            (usage, citations) => {
                ...
                _activeStreams.delete(_streamConvId);
                const _isVisible = (_streamConvId === conversationId) && _streamCtx.assistantDiv && _streamCtx.assistantDiv.isConnected;
                const _canSaveGlobals = _isVisible && (_streamHistory === conversationHistory);
                ...
                // est un nouveau tableau (donc !== _streamHistory), mais l'isStreaming
                ...
                if (_canSaveGlobals) {
                    ...
                    updateSendButton();
                    promptInput.focus();
                    if (window.WorkMode?.canWrite) {
                        window.WorkMode.applyOperations(_streamCtx.accumulatedRaw).then(async operations => {
                            if (!operations.length) return;
                            const report = await window.WorkMode.createReport(operations, activeTextModel);
                            const lastMessage = _streamHistory[_streamHistory.length - 1];
                            if (!lastMessage || lastMessage.role !== 'assistant') return;
                            // Une action Work réussie reçoit un compte rendu applicatif
                            // court : on masque la réponse intermédiaire du modèle et
                            // ses éventuelles balises techniques.
                            lastMessage.content = report;
                            if (_canSaveGlobals && ad) {
                                ad._rawMarkdown = lastMessage.content;
                                const textEl = ad.querySelector('.message-text');
                                if (textEl) { textEl.innerHTML = marked.parse(lastMessage.content); addCodeCopyButtons(textEl); }
                                saveConversation();
                            } else {
                                _saveConvById(_streamConvId, _streamHistory, { modelKey: activeTextModel });
                            }
                        }).catch(error => console.error('Work : traitement impossible', error));
                    }
                } else {
                    _saveConvById(_streamConvId, _streamHistory, {
                        tokensIn: usage?.input_tokens || 0,
            ...
            (thinkChunk) => {
                ...
                if (_streamConvId !== conversationId || !_streamCtx.assistantDiv || !_streamCtx.assistantDiv.isConnected) return;
                ensureThinkBlock();
...
async function refreshConvList() {
    ...
    if (activeCategoryId) {
        const activeCat = readCategory(activeCategoryId);
    ...
    for (const conv of conversations) {
        ...
        catBtn.title = 'Changer de projet';
...
function refreshConvListItem(filename) {
    ...
    if (activeCategoryId && meta.category !== activeCategoryId) {
        existing.remove();
        return true;
    }
...
function showCatPopup(btn, filename, currentCategoryId) {
    ...
    noCatItem.innerHTML = '<span style="opacity:0.4;font-size:0.8rem">—</span><span>Sans projet</span>';
...
async function deleteConversation(filename) {
    ...
    for (const [convId, ctx] of _activeStreams.entries()) {
...
async function loadConversation(filename) {
    // Ne pas abandonner les streams en cours : ils continueront en arrière-plan
    // et persisteront leur résultat sur disque via leur conversationId capturé.
    ...
    const _currentFilename = conversationId
        ? conversationId.replace(/[<>:"/\\|?*]/g, '_') + '.json'
        : null;
    ...
    conversationLastActivity = data.lastActivity || data.date;
    ...
    totalAudioCost = data.cout_audio || 0;
    totalTitleCost = data.cout_titre || 0;
    costByModel = data.cost_by_model || {};
    currentConversationCategory = data.category || null;
    window.WorkMode?.restoreConversation?.(data.work_folder || data.workFolder || null);
    if (conversationHistory.length > 0) {
        const _firstContent = conversationHistory[0].content;
        const _firstText = getTextFromContent(_firstContent);
        if (_firstText) {
            firstPrompt = _firstText;
            ...
            if (_imgPart) firstPrompt = '(image)';
            else if (_files.length === 1 && _filePart?.name) firstPrompt = `(${_filePart.name})`;
            else if (_files.length > 1) firstPrompt = '(fichiers joints)';
            else firstPrompt = '(message)';
        } else {
            firstPrompt = '(message)';
        }
    } else {
    ...
    if (isImageModel) {
        ...
        // Ne pas l'assigner à currentModel, l'utilisateur doit en choisir un autre avant d'envoyer.
        currentModel = null;
        currentImageModel = null;
        currentSearchModel = null;
        _switchTab('text', false);
    ...
    if (lastSwitch) {
        ...
        if (isImg || isSrch || isTxt) {
            currentModel = null; currentImageModel = null; currentSearchModel = null;
            if (isImg) {
                currentImageModel = restoredModel;
                _switchTab('image', false);
            } else if (isSrch) {
                currentSearchModel = restoredModel;
                _switchTab('search', false);
            } else {
    ...
    const _liveStream = _activeStreams.get(conversationId);
    if (_liveStream) {
        isStreaming = true;
        currentAbortController = _liveStream.abortController;
...
async function refreshSpList() {
    ...
    spSelect.innerHTML = '<option value="">Sélectionner un agent enregistré</option>';
    for (const sp of prompts) {
        const opt = document.createElement('option');
...
async function deleteSpItem(filename, nom) {
    if (!await customConfirm(`Supprimer l'agent "${nom}" ?`, { icon: 'delete', danger: true, okLabel: 'Supprimer' })) return;
...
spImportFile.addEventListener('change', async () => {
    ...
    try {
        ...
        if (!data._minou_role || !data.nom || !data.contenu) {
            showModelAlert('Ce fichier n\'est pas un agent Kiro valide.');
            return;
        }
        ...
        console.error('Erreur import agent:', e);
        showModelAlert('Erreur lors de l\'import de l\'agent.');
    }
});
...
function openSpModal(filename = null, fromManage = false) {
    ...
    if (filename) {
        ...
        spModalTitle.textContent = 'Nouvel agent';
...
spModalDelete.addEventListener('click', async () => {
    if (!spEditingFilename) return;
    const data = await readSystemPrompt(spEditingFilename);
    const roleName = data ? data.nom : spEditingFilename;
    if (!await customConfirm(`Supprimer l'agent « ${roleName} » ?`, { icon: 'delete', danger: true, okLabel: 'Supprimer' })) return;
    await deleteSystemPromptFile(spEditingFilename);
    if (spSelect.value === spEditingFilename) {
        spSelect.value = '';
        currentSystemPrompt = null;
    }
    refreshSpList();
...
spEditBtn.addEventListener('click', async () => {
    ...
    spEditBtn.style.display = 'none';
    if (currentSystemPrompt) currentSystemPrompt.contenu = contenu;
});
...
spDeleteBtn?.addEventListener('click', async () => {
    ...
    if (currentSystemPrompt && currentSystemPrompt.nom === roleName) {
        currentSystemPrompt = null;
    }
    spSelect.value = '';
    spTextarea.value = '';
...
prModalDelete.addEventListener('click', async () => {
    if (!prEditingFilename) return;
    const data = await readSavedPrompt(prEditingFilename);
    const promptName = data ? data.nom : prEditingFilename;
...
micBtn.addEventListener('click', async () => {
    ...
    if (isStreaming && micBtn.classList.contains('stop-mode')) {
        if (currentAbortController) currentAbortController.abort();
        return;
    }
    // Verrou anti double-clic : pendant la transcription, le mediaRecorder est déjà
...
prImportFile.addEventListener('change', async () => {
    ...
    try {
        ...
        if (!data._minou_prompt || !data.nom || !data.contenu) {
            showModelAlert('Ce fichier n\'est pas un prompt Kiro valide.');
            return;
        }
        const filename = data.nom.replace(/[^a-zA-Z0-9àâéèêëïîôùûüçÀÂÉÈÊËÏÎÔÙÛÜÇ _-]/g, '_') + '.json';
...
async function exportBackup() {
    ...
    const data = {
        ...
        apiKeys: includeKeys ? JSON.parse(localStorage.getItem('minou-apikeys') || '{}') : {},
...
async function importBackup(file) {
    try {
        ...
        if (!await customConfirm(`Importer ${convCount} conversation(s), ${spCount} agent(s), ${prCount} prompt(s) enregistré(s) et ${catCount} catégorie(s) ?\n\nLes données existantes portant les mêmes noms seront écrasées.`, { icon: 'import', okLabel: 'Importer' })) return;
...
function _restoreApiKeyInputs() {
    ...
    document.getElementById('apikey-groq').value = API_KEYS.groq || '';
    document.getElementById('apikey-ollamaCloud').value = API_KEYS.ollamaCloud || '';
    document.getElementById('apikey-ollamaCloudUrl').value = API_KEYS.ollamaCloudUrl || 'https://ollama.com';
    document.getElementById('apikey-openrouter').value = API_KEYS.openrouter || '';
    document.getElementById('apikey-ollama').value = /^https:\/\/ollama\.com\/?$/i.test(API_KEYS.ollama || '')
    ...
    document.getElementById('apikey-omniroute').value = API_KEYS.omniroute || '';
    document.getElementById('apikey-omnirouteUrl').value = API_KEYS.omnirouteUrl || 'http://localhost:20128/v1';
}
...
(function initConfigAutoSave() {
    ...
    const apiKeyIds = ['apikey-openai','apikey-anthropic','apikey-google','apikey-perplexity',
        'apikey-mistral','apikey-deepseek','apikey-grok','apikey-zai','apikey-groq',
        'apikey-ollamaCloud','apikey-ollamaCloudUrl','apikey-openrouter','apikey-ollama','apikey-lmstudio',
        'apikey-omniroute','apikey-omnirouteUrl'
        ];
    ...
    if (apimodelesSaveBtn) {
        apimodelesSaveBtn.addEventListener('click', () => {
            ...
            _refreshSavedProviderCatalog(_activeProvider);
        });
    }
...
const PROVIDERS_CONFIG = [
    ...
    { id: 'groq',       label: 'Groq',             icon: 'Groq.svg',       placeholder: 'gsk-...',    link: 'https://console.groq.com/keys',                             linkLabel: 'Obtenir une clé API Groq' },
    { id: 'ollamaCloud', label: 'Ollama Cloud',    icon: 'Ollama.svg',     placeholder: 'sk-...',     link: 'https://ollama.com',                                       linkLabel: 'Ouvrir Ollama Cloud', hasImage: true },
    { id: 'omniroute', label: 'OmniRoute',          icon: 'Other.svg',      placeholder: 'Clé API OmniRoute', link: 'https://github.com/diegohaz/omniroute', linkLabel: 'Documentation OmniRoute' },
...
const _providerCatalogSearch = {};
const _providerCatalogCategory = {};
...
function _buildProviderSectionHtml(p, isFirst) {
    const isOmniRouteProvider = p.id === 'omniroute';
    const isOllamaCloudProvider = p.id === 'ollamaCloud';
    ...
    const customFields = isOmniRouteProvider ? `
        <div class="apikey-field apikey-field-sub" style="margin-top:10px;">
            <label class="sp-modal-label" for="apikey-omnirouteUrl">URL de base</label>
            <div class="apikey-input-wrap">
                <input type="text" id="apikey-omnirouteUrl" class="sp-modal-input apikey-input" placeholder="http://localhost:20128/v1">
            </div>
        </div>
    ` : '';
    const ollamaCloudFields = isOllamaCloudProvider ? `
        <div class="apikey-field apikey-field-sub" style="margin-top:10px;">
            <label class="sp-modal-label" for="apikey-ollamaCloudUrl">URL de base</label>
            <div class="apikey-input-wrap">
                <input type="text" id="apikey-ollamaCloudUrl" class="sp-modal-input apikey-input" placeholder="https://ollama.com">
            </div>
        </div>
    ` : '';
    return `
...
function _initApiModelesPanel() {
    ...
    PROVIDERS_CONFIG.forEach(p => {
        ...
        const extraInput = section.querySelector(`#apikey-${p.id}Url`);
        if (extraInput) extraInput.addEventListener('input', () => {
            _setKeysDirty(true);
        });
...
function _renderStandardProviderCatalog(container, providerId, models, options = {}) {
    const disabled = _catalogPending.disabled;
    const currentCategory = _providerCatalogCategory[providerId] || 'all';
    const categories = [...new Set(models.map(m => m.category).filter(Boolean))].sort();
    const filterModels = () => models.filter(m => {
        const searchTerm = (_providerCatalogSearch[providerId] || '').trim().toLowerCase();
        const category = _providerCatalogCategory[providerId] || 'all';
    ...
    const allChecked = visibleModels.length > 0 && visibleModels.every(m => !disabled.has(m.id));
    const someChecked = visibleModels.some(m => !disabled.has(m.id)) && !allChecked;
    ...
    section.innerHTML = `
        ...
        <div class="catalog-or-toolbar">
            <div class="catalog-or-cat-select-wrap">
                <select class="catalog-or-cat-select catalog-provider-category">
                    <option value="all">Toutes les catégories</option>
                    ${categories.map(cat => `<option value="${escHtml(cat)}"${cat === currentCategory ? ' selected' : ''}>${escHtml(cat)}</option>`).join('')}
                </select>
            ...
            <div class="catalog-or-search-wrap">
                ...
                <input type="text" class="catalog-or-search provider-catalog-search" placeholder="Rechercher un modèle…" value="${escHtml(_providerCatalogSearch[providerId] || '')}">
            </div>
        </div>
        <div class="catalog-section-models">
            ${visibleModels.length ? visibleModels.map(m => _catalogRowHtml(m, !!options.isImage, false, !disabled.has(m.id))).join('') : '<div class="catalog-or-empty">Aucun modèle ne correspond à votre recherche.</div>'}
        </div>`;
    const modelsContainer = section.querySelector('.catalog-section-models');
    const masterCb = section.querySelector('.catalog-section-master-cb');
    const refreshRows = () => {
        const filtered = filterModels();
        modelsContainer.innerHTML = filtered.length
            ? filtered.map(m => _catalogRowHtml(m, !!options.isImage, false, !disabled.has(m.id))).join('')
            : '<div class="catalog-or-empty">Aucun modèle ne correspond à votre recherche.</div>';
        const enabled = filtered.filter(m => !disabled.has(m.id)).length;
        masterCb.checked = filtered.length > 0 && enabled === filtered.length;
        masterCb.indeterminate = enabled > 0 && enabled < filtered.length;
        attachRowListeners(filtered);
    };
    const attachRowListeners = filtered => {
        modelsContainer.querySelectorAll('.catalog-cb').forEach(cb => {
            cb.addEventListener('change', () => {
                if (cb.checked) disabled.delete(cb.dataset.id);
                else disabled.add(cb.dataset.id);
    ...
    masterCb.addEventListener('change', () => {
        const filtered = filterModels();
        if (masterCb.checked) filtered.forEach(m => disabled.delete(m.id));
        else filtered.forEach(m => disabled.add(m.id));
        refreshRows();
        _refreshCatalogDirty();
    });
    const categorySelect = section.querySelector('.catalog-provider-category');
    categorySelect.addEventListener('change', () => {
        _providerCatalogCategory[providerId] = categorySelect.value;
        refreshRows();
    });
    const searchInput = section.querySelector('.provider-catalog-search');
    searchInput.addEventListener('input', () => {
        _providerCatalogSearch[providerId] = searchInput.value;
        refreshRows();
    });
    section.querySelectorAll('.catalog-type-btn').forEach(button => {
        button.addEventListener('click', () => {
            if (button.classList.contains('active')) return;
            section.querySelectorAll('.catalog-type-btn').forEach(other => other.classList.toggle('active', other === button));
            _providerCatalogType[providerId] = button.dataset.type;
            setTimeout(() => renderProviderCatalog(providerId), 260);
        });
    });
    attachRowListeners(visibleModels);
...
function renderProviderCatalog(providerId) {
    ...
    if (providerId === 'groq' || providerId === 'omniroute' || providerId === 'ollamaCloud') {
        if (!hasProviderKey(providerId)) {
            const msg = providerId === 'groq'
                ? 'Renseignez votre clé API Groq pour charger le catalogue de modèles.'
                : providerId === 'ollamaCloud'
                    ? 'Renseignez votre clé API Ollama Cloud et l’URL de base pour charger le catalogue.'
                    : providerId === 'omniroute'
                        ? 'Renseignez votre clé API OmniRoute et l’URL de base pour charger le catalogue.'
                        : 'Renseignez votre clé API et l’URL de base pour charger le catalogue de modèles.';
            container.innerHTML = `<div class="provider-models-empty">${msg}</div>`;
            return;
        }
        const allDynamicModels = MODELS.filter(m => m.editeur === providerId);
        if (allDynamicModels.length === 0) {
            container.innerHTML = '<div class="provider-models-empty">Aucun modèle chargé. Cliquez sur « Sauvegarder » pour actualiser le catalogue.</div>';
            return;
        }
        _renderStandardProviderCatalog(container, providerId, allDynamicModels);
        return;
    }
    ...
    if (provider.isLocal) {
        ...
        _renderStandardProviderCatalog(container, providerId, localModels);
        return;
    }
    ...
    _renderStandardProviderCatalog(container, providerId, baseModels, { isImage, typeToggle });
}
...
const _STORAGE_VIEWMODE_KEY = 'kiro-storage-media-viewmode';
const _storage = {
...
async function _downloadSelectedStorage() {
    ...
    try {
        ...
        _triggerDownload(zipBlob, `kiro-medias-${stamp}.zip`);
    } catch (e) {
        ...
        while (i < models.length) {
            ...
            try {
                ...
                const res = await fetch(url, {
                    headers: { 'Authorization': `Bearer ${API_KEYS.openrouter}`, 'HTTP-Referer': 'https://hinata.local/', 'X-Title': 'Hinata' }
                });
                if (!res.ok) continue;
...
async function _refreshSavedProviderCatalog(providerId) {
    if (!(providerId === 'groq' || providerId === 'omniroute' || providerId === 'ollamaCloud')) return;
    try {
        await refreshProviderCatalogFromApi(providerId);
    } catch (error) {
        console.warn(`[Kiro] Catalogue ${providerId} indisponible :`, error);
    }
    if (typeof populateModelSelect === 'function') populateModelSelect();
    if (document.getElementById(`catalog-list-${providerId}`)) {
        renderProviderCatalog(providerId);
    }
}
...
function closeLightbox() {
    if (lightboxOverlay.style.display === 'none' || lightboxOverlay.classList.contains('closing')) return;
    lightboxOverlay.classList.add('closing');
    const onEnd = (e) => {
        if (e.target !== lightboxOverlay) return;
        lightboxOverlay.removeEventListener('animationend', onEnd);
        lightboxOverlay.style.display = 'none';
        lightboxOverlay.classList.remove('closing');
        lightboxImg.src = '';
    };
    lightboxOverlay.addEventListener('animationend', onEnd);
}
...
function openFileViewer(blobUrl, fileName) {
    fileViewerTitle.textContent = fileName;
    fileViewerIframe.src = blobUrl;
    fileViewerOverlay.classList.remove('closing');
    fileViewerOverlay.style.display = 'flex';
}

function closeFileViewer() {
    if (fileViewerOverlay.style.display === 'none' || fileViewerOverlay.classList.contains('closing')) return;
    fileViewerOverlay.classList.add('closing');
    const onEnd = (e) => {
        if (e.target !== fileViewerOverlay) return;
        fileViewerOverlay.removeEventListener('animationend', onEnd);
        fileViewerOverlay.style.display = 'none';
        fileViewerOverlay.classList.remove('closing');
        fileViewerIframe.src = '';
    };
    fileViewerOverlay.addEventListener('animationend', onEnd);
}
...
if (fileViewerOverlay) {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && fileViewerOverlay.style.display !== 'none') closeFileViewer();
    });
}

// --- Easter egg : taper "Kiro" dans la zone de saisie ---
(function () {
    // Le trigger ne doit pas se déclencher sur "monkiro", "akiro", etc.
    // → exiger un début de chaîne ou un caractère non-lettre juste avant.
    const TRIGGER_RE = /(^|[^\p{L}])kiro$/u;
...
// Vérification de mise à jour au chargement
(async function checkForUpdate() {
    try {
        ...
        if (remoteVersion && remoteVersion !== APP_VERSION) {
            ...
            document.getElementById('update-toast-version').textContent = 'v' + remoteVersion;
            toast.style.display = 'flex';
            toast.querySelector('.update-toast-close').addEventListener('click', () => {
                toast.style.display = 'none';
            });
        }
    } catch (_) { /* silencieux si réseau indisponible */ }
})();
...
function initSidebarResize() {
    ...
    const savedWidth = localStorage.getItem('kiro-sidebar-width');
    if (savedWidth) {
        const w = Math.max(180, Math.min(500, parseInt(savedWidth, 10)));
