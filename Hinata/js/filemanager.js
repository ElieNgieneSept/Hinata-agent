...
function isDesktopStorageAvailable() {
    return !!window.kiroDesktop?.conversations;
}

async function migrateBrowserConversationsToDesktop() {
    if (!isDesktopStorageAvailable() || window.kiroDesktop.conversations.list().length > 0) return;
    try {
        const db = await openConvDB();
        const tx = db.transaction('conversations', 'readonly');
        const store = tx.objectStore('conversations');
        const [keys, values] = await Promise.all([
            new Promise(resolve => { const req = store.getAllKeys(); req.onsuccess = () => resolve(req.result); req.onerror = () => resolve([]); }),
            new Promise(resolve => { const req = store.getAll(); req.onsuccess = () => resolve(req.result); req.onerror = () => resolve([]); })
        ]);
        keys.forEach((key, index) => {
            if (values[index] != null) window.kiroDesktop.conversations.write(key, values[index]);
        });
    } catch (error) { console.warn('Migration conversations desktop impossible :', error); }
}
...
async function writeConversationFile(filename, content) {
    if (isDesktopStorageAvailable()) {
        try {
            window.kiroDesktop.conversations.write(filename, typeof content === 'string' ? JSON.parse(content) : content);
            _afterConvWrite(filename, content);
            return true;
        } catch (e) { console.error('Erreur écriture conversation desktop:', e); return false; }
    }
...
async function listConversationFiles(includeDeleted = false) {
    try {
        if (isDesktopStorageAvailable()) {
            await migrateBrowserConversationsToDesktop();
            const conversations = [];
            for (const filename of window.kiroDesktop.conversations.list()) {
                const data = window.kiroDesktop.conversations.read(filename);
                if (!data) continue;
                _updateConvMetaInManifest(filename, data);
                const meta = _convMetaManifest.get(filename);
                if (meta && (includeDeleted || !meta.deleted)) conversations.push({ ...meta });
            }
    ...
    try {
        if (isDesktopStorageAvailable()) {
            ...
            for (const filename of window.kiroDesktop.conversations.list()) {
                ...
                result[filename] = (data.messages || []).map(msg => {
...
async function updateConversationFile(filename, mergeFn) {
    if (isDesktopStorageAvailable()) {
        try {
            const current = window.kiroDesktop.conversations.read(filename);
            const updated = await mergeFn(current);
            if (updated == null) return false;
            window.kiroDesktop.conversations.write(filename, updated);
            _afterConvWrite(filename, updated);
            return true;
        } catch (e) { console.error('Erreur update conversation desktop:', e); return false; }
    }
    const prev = _writeQueues.get(filename) || Promise.resolve();
    const next = prev.then(async () => {
        try {
...
async function readConversationFile(filename) {
    if (isDesktopStorageAvailable()) {
        try { return window.kiroDesktop.conversations.read(filename); } catch (e) { console.error('Erreur lecture conversation desktop:', e); return null; }
    }
    try {
        const db = await openConvDB();
        const tx = db.transaction('conversations', 'readonly');
...
async function writeSystemPrompt(filename, data) {
    ...
    store[filename] = {
        ...
        mascottePosition: data.mascottePosition !== undefined ? data.mascottePosition : (previous.mascottePosition || null)
    };
    _saveSpStore(store);
    return true;
}
...
function formatConversationFile(data) {
    ...
    if (data.workFolder) obj.work_folder = data.workFolder;
